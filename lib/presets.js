...
const PRESETS = {
  // Long-running Node host: rsync the repo + (re)start pm2 processes over SSH (editor, analytics).
  'server-ssh': {
  ...
  // Static artifact synced to S3, optional CloudFront invalidation (player).
  'static-s3': {
    deploy: { strategy: 'static-s3', cacheControl: 'no-cache', delete: true },
    channels: { production: { stampPath: 'build-stamp.json', release: 'minor' } },
  },
  // Library consumed via git-tag (adapters, SCORM-Builder): the tag IS the release.
  // No live URL → no server deploy; `verify` falls back to a tag/commit check.
  library: {
    deploy: { strategy: 'library' },
    channels: {},
  },
  // Repo-provided deploy command (migrator): the toolkit owns version/tag/stamp/verify,
